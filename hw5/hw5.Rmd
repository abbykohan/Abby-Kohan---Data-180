...
```{r}
Dist_V <- dist(Vans93[,-1])
Vans_Euclidean <-round(as.matrix(Dist_V), 2)
colnames(Vans_Euclidean)<-Vans93[,1]
rownames(Vans_Euclidean)<-Vans93[,1]
Vans_Euclidean
```
...
```{r}
Dist_M <- dist(Vans93[,-1], method="manhattan")
Vans_Manhattan <-round(as.matrix(Dist_M), 2)
...
```{r}
vertebrate_euclidean <- dist(vertebrate [,-1])
Ver_Euclidean<-as.matrix(dist(vertebrate[,-1]))
colnames(Ver_Euclidean)<-vertebrate[,1]
rownames(Ver_Euclidean)<-vertebrate[,1]
plot(hclust(dist(Ver_Euclidean)), lwd=3, main="Clustered vertebrates, Metric type,\n Linkage type", sub="", xlab="Vertebrates", cex.lab=1.25, hang=-1)
...
plot(hclust(dist(Ver_Manhattan), method="single"), lwd=3, main="Clustered vertebrates, Metric type,\n Linkage type", sub="", xlab="Vertebrates", cex.lab=1.25, hang=-1)
```

Which method do you prefer?
I prefer Euclidean because it gives more defined and more specific clusters and is easier to read personally. 
