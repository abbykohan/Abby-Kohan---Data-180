...
```{r}
M_HC <- hclust(dist(Mammals_Milk_Std))
plot(M_HC, lwd=3, main="Mammals Clustered by Std. Milk Composition \n Euclidean Metric, Complete Linkage", sub="", cex.lab=1.25, hang=-1,frame.plot=T, xlab="Mammals")
```
...
```{r}
MMilk_Agg_K3 <- Mammals_Milk %>% group_by(km_3) %>% summarize(Mean_Water = mean(Water), Mean_Protein = mean(Protein), Mean_Fat = mean(Fat), Mean_Lactose = mean(Lactose), Mean_Ash = mean(Ash), Count = n()) %>% round(2)
MMilk_Agg_K4 <- Mammals_Milk %>% group_by(km_4) %>% summarize(Mean_Water = mean(Water), Mean_Protein = mean(Protein), Mean_Fat = mean(Fat), Mean_Lactose = mean(Lactose), Mean_Ash = mean(Ash), Count = n()) %>% round(2)
...
#The plot suggests there are about 3-4 clusters because the line bends at around x=3 and x=4
#This is accurate to the dendrogram because there appear to be around 3 main clusters which is similar to the elbow plot.

# Very well interpreted. Good job. 
```
