...
library(tidyverse)
library(tm)
news<-read.csv("~/Desktop/Abby Kohan - Data 180/Abby-Kohan---Data-180/hw7/news.csv", header=T)
```

Read also the positive and negative word dictionaries, both found on Github. This will later come in handy:

```{r}
posWords <- scan("~/Desktop/Abby Kohan - Data 180/Abby-Kohan---Data-180/hw7/positive-words.txt", character(0), sep = "\n")  # 2006 items
negWords <- scan("~/Desktop/Abby Kohan - Data 180/Abby-Kohan---Data-180/hw7/negative-words.txt", character(0), sep = "\n")  # 4783 items
head(posWords,15)
head(negWords,15)
```
...
```{r}
#term_stats(charVector, ngrams=1, filter=text_filter(drop_punc=TRUE, drop_symbol=TRUE, drop=stopwords_en))
```
