...
```{r}
ggplot(Cars93, aes(x=EngineSize,y=MPG.city) ) + geom_point(col = "blue", size = 2) + 
                          labs(y = 'Engine size', x = 'MPG city', title = "Engine Size vs MPG City", subtitle = "Scatterplot") + xlim (1,6) + ylim(10,50)
```
...
```{r}
ggplot(Cars93, aes(x=EngineSize,y=MPG.city, colour = Type) ) + geom_point(size = 2) + 
                          labs(y = 'Engine size', x = 'MPG city', title = "Engine Size vs MPG City", subtitle = "Scatterplot") + xlim (1,6) + ylim(10,50)
```
...
```{r}
ggplot(Cars93, aes(x=EngineSize,y=MPG.city, colour = Type) ) + geom_point(size = 2) + 
                          labs(y = 'Engine size', x = 'MPG city', title = "Engine Size vs MPG City", subtitle = "Scatterplot") + xlim (1,6) + ylim(10,50) + geom_smooth(method = "loess", color = "red", linetype = "dashed")
```
...
ggplot(Cars93, aes(x = AirBags, fill = AirBags)) +
  geom_bar() +
  labs( title = "Barplot of Airbags", subtitle = "barplot", x = "Airbags", y = "Count")
```
## Task 5
...
```{r}
ggplot(data = au_states) + geom_sf() + labs(title = "Map of Australia")
```
...
```{r}
ggplot(data = au_states, aes(fill = NAME)) + geom_sf() + labs (title = "Map of Australia")
```
## Task 3
...
```{r}
ggplot(data = au_states) + geom_sf() + geom_point(data = au_capitals, col = "red", size = 2, aes(x = lon, y = lat)) + labs (title = "Map of Australia")
```
...
```{r}
ggplot(data = au_states) + geom_sf() + geom_point(data = au_capitals, col = "red", size = 2, aes(x = lon, y = lat))+labs (title = "Map of Australia") + coord_sf(crs = st_crs(3112))
```
...
```{r}
library(mapview)
mapview(au_states) + mapview(capitals_geo, col.regions = "red", legend=FALSE)
```
