...
```{r}
mk_q1 <- (count(filter(records, record_duration > 365)))
mk_q1
```
# Question 3
...
```{r}
mk_q3 <- records %>% select(player, track, date, record_duration) %>% filter(record_duration == max(record_duration))
mk_q3
...
```{r}
mk_q4 <- records %>% group_by(track) %>% summarize(mean_time = mean(time)) %>% arrange(mean_time)
mk_q4
```
# Question 5
...
```{r}
mk_q5 <- records %>% mutate(year = substr(date, 1, 4))
head(mk_q5)
```
...
```{r}
mk_6 <- mk_q5 %>% group_by(date) %>% summarize(mean_time = mean(time)) %>% arrange(mean_time)
mk_6
```
# Question 7
...
```{r, fig.width=10}
#the times got faster as the years went on. The record times are getting faster.

# There is no trend. 

mk_7 <- mk_q5 %>% group_by(year) %>% summarize(mean_time = mean(time)) %>% arrange(mean_time) %>%  ggplot(aes(year,mean_time)) + geom_bar(stat = "identity")
mk_7
```
...
```{r}
mk_q8 <- mk_q5 %>%  ggplot(.,aes(fill=type, x=time)) + geom_histogram(alpha=0.5)
mk_q8
```
...
```{r}
mk_q9 <- mk_q5 %>% mutate(time_per = ifelse(type=="Three Lap",time/3,time)) %>% ggplot(.,aes(fill=type, x=time_per)) + geom_histogram(alpha=0.5)
mk_q9
```
# Question 10
...
```{r}
mk_q10 <- records %>% filter(shortcut=="No") %>% mutate(time_per = ifelse(type=="Three Lap",time/3,time))  %>% ggplot(.,aes(fill=type, x=time_per)) + geom_histogram(alpha=0.5)
mk_q10
```
...
```{r}
mk_q11 <- records %>% group_by(system_played) %>% summarize(mean_time = mean(time)) 
mk_q11
#the mean time for NTSC is significantly smaller than the mean time for PAL. The fact that NTSC runs at 60 FPS could very well have an affect on the mean time since the PAL system runs at a slower FPS and has longer mean times. 
```
